...
namespace QwMicroKernel.Logging
{
    ...
    {
        readonly ICache<string, TraceLogWriter> _logs;
        readonly ICache<string, TraceSource> _sources;
        TraceListener _listener;
        readonly TraceSource _defaultSource;

        public TraceLogWriterFactory()
        {
            _logs = new QwDictionary<string, TraceLogWriter>(CreateTraceLog);
            _sources = new QwDictionary<string, TraceSource>(CreateTraceSource);

            _defaultSource = new TraceSource("Default", SourceLevels.Information);

            _listener = AddDefaultConsoleTraceListener(_defaultSource);

            _sources.Get("QKernel");
        }
        ...
        {
            var listener = new ConsoleTraceListener { Name = "Q.Net" };
