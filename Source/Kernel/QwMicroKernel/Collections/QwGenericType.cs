...
{
    public class QwGenericType<TInterface> : ICache<Type, TInterface>
    {
        readonly ICache<Type, TInterface> _cache;
        readonly Type _genericType;

        QwGenericType(Type genericType, ICache<Type, TInterface> cache)
        {
#if !NETFX_CORE
        ...
        public QwGenericType(Type genericType)
#if NET35
...
#else
            : this(genericType, new QwConcurrent<Type, TInterface>(DefaultMissingValueProvider(genericType)))
#endif
        {
        }
        ...
        public QwGenericType(Type genericType, MissingValueProvider<Type, TInterface> missingValueProvider)
#if NET35
            : this(genericType, new ReaderWriterLockedCache<Type, TInterface>(
                new DictionaryCache<Type,TInterface>(missingValueProvider)))
#else
            : this(genericType, new QwConcurrent<Type, TInterface>(missingValueProvider))
#endif
        {
        }
