...
namespace QwMicroKernel.Collections
{
    ...
    [Serializable]
#endif
    public class QwDictionary<TKey, TValue> : ICache<TKey, TValue>
    {
        ...
        public QwDictionary(MissingValueProvider<TKey, TValue> missingValueProvider)
            : this()
        {
            _missingValueProvider = missingValueProvider;
        }

        public QwDictionary(IEqualityComparer<TKey> equalityComparer)
        {
            _values = new Dictionary<TKey, TValue>(equalityComparer);
        }

        public QwDictionary(KeySelector<TKey, TValue> keySelector)
        {
            _values = new Dictionary<TKey, TValue>();
            _keySelector = keySelector;
        }

        public QwDictionary(KeySelector<TKey, TValue> keySelector, IEnumerable<TValue> values)
            : this(keySelector)
        {
            Fill(values);
        }
        ...
        public QwDictionary(IEnumerable<KeyValuePair<TKey, TValue>> values)
        {
            _values = values.ToDictionary(x => x.Key, x => x.Value);
        }

        public QwDictionary(IDictionary<TKey, TValue> values) :
            this(values, true)
        {
        }

        public QwDictionary(IDictionary<TKey, TValue> values, bool copy)
        {
            _values = copy
        ...
        public QwDictionary(IEnumerable<KeyValuePair<TKey, TValue>> values, IEqualityComparer<TKey> equalityComparer)
        {
            _values = values.ToDictionary(x => x.Key, x => x.Value, equalityComparer);
        }

        public QwDictionary(IDictionary<TKey, TValue> values, IEqualityComparer<TKey> equalityComparer)
        {
        ...
        public void Each(Func<TKey, TValue, bool> callback)
        {
            foreach (var value in _values)
                if (callback(value.Key, value.Value))
                    break;
        }
