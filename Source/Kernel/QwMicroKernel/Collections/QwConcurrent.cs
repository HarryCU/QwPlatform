...
namespace QwMicroKernel.Collections
{
    ...
    [Serializable]
#endif
    public class QwConcurrent<TKey, TValue> : ICache<TKey, TValue>
    {
        ...
        public QwConcurrent(MissingValueProvider<TKey, TValue> missingValueProvider)
            : this()
        {
            _missingValueProvider = missingValueProvider;
        }

        public QwConcurrent(IEqualityComparer<TKey> equalityComparer)
        {
            _values = new ConcurrentDictionary<TKey, TValue>(equalityComparer);
        }

        public QwConcurrent(KeySelector<TKey, TValue> keySelector)
        {
            _values = new ConcurrentDictionary<TKey, TValue>();
            _keySelector = keySelector;
        }

        public QwConcurrent(KeySelector<TKey, TValue> keySelector, IEnumerable<TValue> values)
            : this(keySelector)
        {
            Fill(values);
        }
        ...
        public QwConcurrent(IEnumerable<KeyValuePair<TKey, TValue>> values, IEqualityComparer<TKey> equalityComparer)
        {
